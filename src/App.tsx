import './App.css'
import { Box, Button, Card, Flex, IconButton, Popover, SimpleGrid, Spinner, useBreakpointValue, } from '@chakra-ui/react'
...
function App() {
  ...
  const popoverPlacement = useBreakpointValue<{ placement: 'right' } | undefined>({ base: undefined, md: { placement: 'right' } });
    ...
    <>
      ...
      >
        ...
        <Card.Root width={"300px"}>
            ...
            <Flex alignItems='center' justifyContent='space-between'>
              ...
              <Popover.Root positioning={popoverPlacement}>
                <Popover.Trigger asChild>
                  <IconButton disabled={ble.status !== 'connected'} size="xs" variant="outline"><VscInfo /></IconButton>
                </Popover.Trigger>
                <Popover.Positioner>
                  <Popover.Content>
          ...
          <Card.Footer display='flex' flexDirection='column' alignItems='stretch'>
            <Button disabled={ble.status !== 'disconnected' && ble.status !== 'connected'} onClick={onClick}>{buttonContent}</Button>
          </Card.Footer>
        </Card.Root>
      </Flex>
    </>
